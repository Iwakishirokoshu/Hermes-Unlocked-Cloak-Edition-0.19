...
def _scan_assembled_cron_prompt(
    ...
    *,
    ...
    """Apply selective safety checks to the assembled cron prompt. Raises
    ``CronPromptInjectionBlocked`` when a match fires so ``run_job`` can
    surface a clear refusal to the operator.

    The user prompt is checked at creation/update time and again here.
    Runtime-loaded skill content is checked for secret access/exfiltration,
    SSH persistence, and destructive commands. Behavioral prompt-injection
    text is allowed by the selective-unlock policy.

    Runtime data from job scripts and upstream jobs is not treated as an
    operator-authored instruction. It is sanitized for invisible Unicode, and
    the original user prompt is independently checked, so quoted commands in a
    report do not disable the job.

    When a job includes both skills and injected data, the skill scan remains
    active. The user-supplied prompt is always checked directly as a runtime
    defense-in-depth for jobs that predate create-time validation.
    """
    ...
    elif has_injected_data:
        assembled, removed = _strip_invisible_unicode(assembled)
        if removed:
            logger.warning(
                "Cron job '%s': stripped runtime-data invisible Unicode (%s)",
                job.get("name") or job.get("id") or "<unknown>",
                ", ".join(removed),
            )
        scan_error = _scan_cron_prompt(user_prompt or "")
    else:
        scan_error = _scan_cron_prompt(assembled)
    if scan_error:
        job_label = job.get("name") or job.get("id") or "<unknown>"
        logger.warning(
            "Cron job '%s': assembled prompt blocked by safety scanner — %s",
            job_label,
            scan_error,
        )
