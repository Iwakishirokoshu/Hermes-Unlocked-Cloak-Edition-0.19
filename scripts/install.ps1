...
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19/main/scripts/install.ps1)
#
...
param(
    [switch]$NoVenv,
    [switch]$SkipSetup,
    # Keeps the Node/CDP client, but deliberately skips a separate local
    # Playwright Chromium download. Used by the explicit Cloak provider,
    # which attaches to the Manager-owned browser over CDP.
    [switch]$SkipBrowser,
    [string]$Branch = "main",
...
$RepoUrlSsh = "git@github.com:Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19.git"
$RepoUrlHttps = "https://github.com/Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19.git"
$PythonVersion = "3.11"
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
...
function Install-NodeDeps {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        if ($browserNpmOk -and -not $SkipBrowser) {
            ...
            Write-Info "Skipping local Playwright Chromium (-SkipBrowser); Cloak connects to the Manager-owned browser over CDP."
        }
    }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/Iwakishirokoshu/Hermes-Unlocked-Cloak-Edition-0.19/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
