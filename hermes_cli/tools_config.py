...
def _run_post_setup(post_setup_key: str):
    ...
    elif post_setup_key == "cloak":
        import subprocess
        installer_sh = PROJECT_ROOT / "scripts" / "install_cloak.sh"
        installer_ps1 = PROJECT_ROOT / "scripts" / "install_cloak.ps1"
        if sys.platform == "win32" and installer_ps1.exists():
            _print_info("    Provisioning Cloak (Windows)...")
            try:
                result = subprocess.run(
                    ["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass",
                     "-File", str(installer_ps1)],
                    text=True, cwd=str(PROJECT_ROOT), timeout=1200,
                )
                if result.returncode == 0:
                    _print_success("    Cloak stack installed")
                else:
                    _print_warning("    Cloak installer returned non-zero — see output above")
            except Exception as exc:
                _print_warning(f"    Cloak installer failed: {exc}")
        elif sys.platform != "win32" and installer_sh.exists() and shutil.which("bash"):
            _print_info("    Provisioning Cloak (cloakbrowser + playwright)...")
            try:
                result = subprocess.run(
                    ["bash", str(installer_sh)],
                    text=True, cwd=str(PROJECT_ROOT), timeout=1200,
                )
                if result.returncode == 0:
                    _print_success("    Cloak stack installed")
            ...
            _print_info("    Installing Cloak Python deps (cloakbrowser, playwright)...")
            try:
                result = subprocess.run(
                    [sys.executable, "-m", "pip", "install", "--quiet",
                     "cloakbrowser>=0.3", "playwright>=1.53", "httpx>=0.27"],
                    capture_output=True, text=True, timeout=900,
                )
                if result.returncode == 0:
                    _print_success("    Cloak Python deps installed")
                else:
                    _print_warning("    pip install failed — install manually: "
                                   "pip install cloakbrowser playwright httpx")
            except Exception as exc:
                _print_warning(f"    Cloak deps install failed: {exc}")
        _print_info("    Next: start CloakBrowser-Manager and set CLOAK_MANAGER_URL "
                    "(default http://127.0.0.1:8080).")
        _print_info("    Then `browser.cloud_provider: cloak` routes browser_* tools "
                    "through your stealth profiles.")
...
_POST_SETUP_READY: dict = {
    ...
    "cloak": lambda: True,  # soft ready — manager may be remote
    "cua_driver": lambda: bool(shutil.which(_cua_driver_cmd())),
}
