...
def mount_spa(application: FastAPI):
    ...
    def _serve_index(prefix: str = ""):
        ...
        # Cloak Edition: floating launcher (/cloak)
        cloak_href = f"{prefix}/cloak"
        cloak_launcher = (
            f'<a href="{cloak_href}" target="_blank" rel="noopener" '
            f'title="Cloak Manager — proxies, captcha keys, token, profiles" '
            f'style="position:fixed;right:18px;bottom:18px;z-index:2147483600;'
            f'display:flex;align-items:center;gap:8px;padding:10px 16px;'
            f'background:#1f6feb;color:#fff;font:600 13px/1 ui-sans-serif,system-ui,'
            f'Segoe UI,Roboto,sans-serif;border-radius:999px;text-decoration:none;'
            f'box-shadow:0 6px 20px rgba(0,0,0,.35);">'
            f'<span style="font-size:15px;">&#9763;</span> Cloak Manager</a>'
        )
        html = html.replace("</body>", f"{cloak_launcher}</body>", 1)
        return HTMLResponse(
            html,
...
# Cloak Edition: panel /cloak + /api/cloak/* (before SPA catch-all!)
try:
    app.state.legacy_session_token = _SESSION_TOKEN
except Exception:
    pass
try:
    from hermes_cli.cloak_dashboard import router as _cloak_router
    app.include_router(_cloak_router)
    _log.info("Mounted Cloak Manager panel: /cloak")
except Exception as _cloak_exc:
    _log.warning("Cloak Manager panel not mounted: %s", _cloak_exc)
