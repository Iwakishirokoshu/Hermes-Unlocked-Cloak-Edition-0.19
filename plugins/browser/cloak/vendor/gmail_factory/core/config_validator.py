...
def validate_config():
    ...
    # Password — empty YOUR_PASSWORD is OK: batch_runner auto-generates one per account.
    if not Config.YOUR_PASSWORD:
        ...
        if os.path.exists(pw_file):
            ...
            if not pw:
                warnings.append(
                    "YOUR_PASSWORD empty and config/password.txt empty — "
                    "a unique random password will be generated per account"
                )
        else:
            warnings.append(
                "YOUR_PASSWORD not set — a unique random password will be "
