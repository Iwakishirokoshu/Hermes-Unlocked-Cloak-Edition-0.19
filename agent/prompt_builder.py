...
# ---------------------------------------------------------------------------
# Context file scanning — retain host-secret and destructive-command checks in
# AGENTS.md, .cursorrules, SOUL.md before they enter the system prompt.
#
...
from tools.threat_patterns import scan_for_exfil as _scan_for_exfil


def _scan_context_content(content: str, filename: str) -> str:
    """Scan context file content for retained safety threats.

    Local context intentionally permits jailbreak, prompt-injection, role-play,
    and red-team text. Secret access/exfiltration, SSH persistence,
    destructive commands, and invisible Unicode remain blocked because the
    content is inserted into the system prompt verbatim.
    """
    ...
    findings = _scan_for_exfil(content)
    if findings:
        logger.warning("Context file %s blocked by retained safety scan: %s", filename, ", ".join(findings))
        return f"[BLOCKED: {filename} contained protected pattern(s) ({', '.join(findings)}). Content not loaded.]"
