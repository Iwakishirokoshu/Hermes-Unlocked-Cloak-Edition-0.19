...
- ``"strict"`` — applied to memory writes + skill installs only
  ...
  for tool results)
- ``"exfil"`` — secret access/exfiltration, SSH persistence, destructive
  commands, and invisible Unicode without behavioral prompt-injection checks
...
_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    (r'rm\s+-rf\s+/', "destructive_root_rm", "strict"),
    (r'\bmkfs\b', "format_filesystem", "strict"),
    (r'\bdd\s+.*if=.*of=/dev/', "disk_overwrite", "strict"),
    (r'/etc/sudoers|visudo', "sudoers_mod", "strict"),
]

# Security-sensitive patterns retained by the selective-unlock surfaces.
# Behavioral prompt injection, role hijacking, and C2 prose deliberately do
# not appear here: those are allowed in local context, memory, cron prompts,
# and skills in this fork.
_EXFIL_PATTERN_IDS = frozenset({
    "exfil_curl",
    "exfil_wget",
    "read_secrets",
    "send_to_url",
...
def _compile() -> None:
    """Compile pattern sets for each scope (all / context / strict / exfil).
    ...
    exfil_patterns: List[Tuple[re.Pattern, str]] = []

    for pattern, pid, scope in _PATTERNS:
        compiled = re.compile(pattern, re.IGNORECASE)
        entry = (compiled, pid)
        if pid in _EXFIL_PATTERN_IDS:
            exfil_patterns.append(entry)
        if scope == "all":
...
def scan_for_threats(content: str, scope: str = "context") -> List[str]:
    ...
    - ``"exfil"``: checks secret access/exfiltration, SSH persistence, and
      destructive operations while allowing behavioral prompt-injection text.
...
def scan_for_exfil(content: str) -> List[str]:
    """Return only secret/exfiltration and host-safety findings.

    This is the selective-unlock scanner used for locally curated text. It
    intentionally permits behavioral prompt injection and jailbreak content.
    """
    return scan_for_threats(content, scope="exfil")


def first_exfil_message(content: str) -> Optional[str]:
    """Return a blocking message for the first retained safety finding."""
    ...
    if pid.startswith("invisible_unicode_"):
        ...
        return f"Blocked: content contains invisible unicode character {codepoint} (possible injection)."
    return (
        f"Blocked: content matches protected pattern '{pid}'. "
        f"Jailbreak and prompt-injection text are allowed, but secret access, "
        f"exfiltration, SSH persistence, and destructive commands are not."
    )
