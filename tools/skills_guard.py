...
based on both the scan verdict and the source's trust level. Behavioral
prompt-injection findings remain visible in the report but do not block
installation in this selective-unlock fork.
...
SCANNER_VERSION = "skills-guard-v2-selective-unlock"
...
def _determine_verdict(findings: List[Finding]) -> str:
    """Determine the install verdict while preserving all scan findings.

    Jailbreak and behavioral prompt-injection text is allowed in skills, but
    every other high- or critical-severity finding retains the stock blocking
    behavior. Invisible Unicode is tagged ``injection`` for reporting
    purposes, yet remains enforced because it can conceal any payload.
    """
    enforced_findings = [
        finding
        for finding in findings
        if finding.category != "injection" or finding.pattern_id == "invisible_unicode"
    ]
    if not enforced_findings:
        return "safe"

    has_critical = any(f.severity == "critical" for f in enforced_findings)
    has_high = any(f.severity == "high" for f in enforced_findings)
