...
# ---------------------------------------------------------------------------
# Memory content scanning — retain secret/exfiltration and host-safety checks
# for content that gets injected into the system prompt.
#
# Patterns live in ``tools/threat_patterns.py`` — the single source of truth
# shared with the context-file scanner and the tool-result delimiter system.
# Memory uses the selective ``exfil`` scope: jailbreak and red-team text is
# allowed to persist in the frozen system-prompt snapshot, while secret access,
# exfiltration, SSH persistence, destructive commands, and invisible Unicode
# remain blocked.
# ---------------------------------------------------------------------------

from tools.threat_patterns import first_exfil_message as _first_exfil_message


def _scan_memory_content(content: str) -> Optional[str]:
    """Scan memory content for retained safety threats. Returns an error if blocked."""
    return _first_exfil_message(content)
...
class MemoryStore:
    ...
    def load_from_disk(self):
        ...
        entry for secret access/exfiltration and host-safety patterns at
        snapshot-build time. A match replaces the entry text in the snapshot
        with a placeholder like ``[BLOCKED: …]``. Behavioral prompt injection
        and jailbreak text is intentionally preserved.
    ...
    def _sanitize_entries_for_snapshot(entries: List[str], filename: str) -> List[str]:
        ...
        ``"exfil"`` scope (same as memory writes). On match, the entry is
        ...
        """
        from tools.threat_patterns import scan_for_exfil
