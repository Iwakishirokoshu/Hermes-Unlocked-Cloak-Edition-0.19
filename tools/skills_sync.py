...
# Dashboard and gateway containers can share HERMES_HOME and run their
# stage-two hooks concurrently. A single exclusive lock protects the manifest
# and every move/copy operation performed during a sync.
SKILLS_SYNC_LOCK_NAME = ".skills-sync.lock"
SKILLS_SYNC_LOCK_TIMEOUT_SECONDS = 60.0
_skills_sync_thread_lock = threading.RLock()


@contextmanager
def _skills_sync_lock():
    """Serialize bundled-skill syncs across threads and processes."""
    lock_path = HERMES_HOME / SKILLS_SYNC_LOCK_NAME
    lock_path.parent.mkdir(parents=True, exist_ok=True)

    with _skills_sync_thread_lock:
        with lock_path.open("a+b") as lock_file:
            # Windows locks a byte range, so make sure the first byte exists.
            lock_file.seek(0)
            lock_file.write(b"\0")
            lock_file.flush()
            ...
            locked = False
            try:
                deadline = time.monotonic() + SKILLS_SYNC_LOCK_TIMEOUT_SECONDS
                if os.name == "nt":
                    import msvcrt
                    ...
                    while not locked:
                        try:
                            ...
                            if time.monotonic() >= deadline:
                                raise TimeoutError(
                                    f"timed out waiting for skills sync lock: {lock_path}"
                                )
                            time.sleep(0.05)
                else:
                    import fcntl
                ...
                if locked:
                    if os.name == "nt":
                        try:
                            ...
                            logger.debug("Could not release skills sync lock %s", lock_path, exc_info=True)
                    else:
                        try:
...
def sync_skills(quiet: bool = False) -> dict:
    """Sync bundled skills while holding the shared profile lock."""
...
def _sync_skills_unlocked(quiet: bool = False) -> dict:
    ...
    for skill_name, skill_src in bundled_skills:
        ...
        if skill_name not in manifest:
            ...
            # The user may already have applied the current bundled version
            # manually (or an earlier interrupted sync may have completed its
            # copy before the manifest write). It is safe to re-baseline this
            # exact match; treating it as user-modified leaves a stale warning
            # forever even though the active skill is current.
            if user_hash == bundled_hash:
                manifest[skill_name] = bundled_hash
                skipped += 1
                continue
