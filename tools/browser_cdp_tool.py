...
_RESOLUTION_TASK_ID: ContextVar[Optional[str]] = ContextVar("browser_cdp_resolution_task_id", default=None)
...
def _resolve_cdp_endpoint(task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        effective_task_id = task_id if task_id is not None else _RESOLUTION_TASK_ID.get()
        return (_get_cdp_override(effective_task_id) or "").strip()
    except Exception as exc:  # pragma: no cover — defensive
        logger.debug("browser_cdp: failed to resolve CDP endpoint: %s", _redact_cdp_output(str(exc)))
        return ""
...
def _private_page_guard_error(blocked_url: str, method: str) -> str:
    return tool_error(
        ...
        f"({_redact_cdp_output(blocked_url)}). Raw CDP method {method!r} could expose private "
...
def _browser_cdp_private_guard(
    *,
    ...
    try:
        ...
        if method == "Page.navigate":
            ...
            if target_url and (
                ...
                return tool_error(
                    ...
                    f"internal address ({_redact_cdp_output(target_url)}).",
                    method=method,
                    cdp_docs=CDP_DOCS_URL,
                )
        ...
        if method == "Runtime.evaluate":
            ...
            if blocked_literal:
                return tool_error(
                    ...
                    f"private or internal address ({_redact_cdp_output(blocked_literal)}).",
                    method=method,
                    cdp_docs=CDP_DOCS_URL,
                )
        ...
        logger.debug("browser_cdp: private-page guard probe failed: %s", _redact_cdp_output(exc))
    return None
...
def _cloak_lease_endpoint_for_task(task_id: str) -> str:
    """Return the Cloak lease CDP URL for a task, if it has one."""
    try:
        from plugins.browser.cloak import session_leases
        from tools.browser_tool import _bare_task_id_for_session_key

        keys = (str(task_id), _bare_task_id_for_session_key(task_id))
        for key in dict.fromkeys(keys):
            lease = session_leases.get(key)
            if lease and lease.cdp_url:
                return str(lease.cdp_url)
    except Exception as exc:  # noqa: BLE001
        logger.debug("browser_cdp: Cloak lease lookup failed: %s", _redact_cdp_output(str(exc)))
    return ""


def _is_cloak_lease_endpoint(task_id: str, endpoint: str) -> bool:
    """Return whether ``endpoint`` belongs to this task's Cloak lease."""
    lease_endpoint = _cloak_lease_endpoint_for_task(task_id)
    return bool(lease_endpoint and lease_endpoint.rstrip("/") == endpoint.rstrip("/"))


async def _cdp_call_serialized_for_cloak(
    ws_url: str,
    method: str,
    params: Dict[str, Any],
    target_id: Optional[str],
    ...
    """Run a raw call under the Cloak action gate when it owns the endpoint."""
    if not _is_cloak_lease_endpoint(task_id, ws_url):
        return await _cdp_call(ws_url, method, params, target_id, timeout)

    try:
        from plugins.browser.cloak._impl.browser_pool import hold_cdp_action
    except Exception as exc:  # noqa: BLE001
        logger.debug("browser_cdp: Cloak action gate unavailable: %s", exc)
        return await _cdp_call(ws_url, method, params, target_id, timeout)

    async with hold_cdp_action(ws_url):
        return await _cdp_call(ws_url, method, params, target_id, timeout)
...
def _browser_cdp_via_supervisor(
    ...
    try:
        ...
        return tool_error(
            f"CDP call via supervisor failed: {type(exc).__name__}: {_redact_cdp_output(str(exc))}",
            cdp_docs=CDP_DOCS_URL,
        )
...
async def _browser_cdp_via_supervisor_serialized(
    task_id: str,
    frame_id: str,
    method: str,
    params: Optional[Dict[str, Any]],
    ...
    """Run frame-scoped raw CDP under the Cloak action gate when applicable."""
    endpoint = _cloak_lease_endpoint_for_task(task_id)
    if not endpoint:
        return await asyncio.to_thread(
            _browser_cdp_via_supervisor, task_id, frame_id, method, params, timeout
        )
    ...
    async with hold_cdp_action(endpoint):
        return await asyncio.to_thread(
            _browser_cdp_via_supervisor, task_id, frame_id, method, params, timeout
        )
...
def browser_cdp(
    ...
    if frame_id:
        ...
        if blocked:
            return blocked
        return _run_async(
            _browser_cdp_via_supervisor_serialized(
                task_id=effective_task_id,
                frame_id=frame_id,
                method=method,
                params=params,
    ...
    resolution_token = _RESOLUTION_TASK_ID.set(effective_task_id)
    try:
        endpoint = _resolve_cdp_endpoint()
    finally:
        _RESOLUTION_TASK_ID.reset(resolution_token)
    if not endpoint:
        return tool_error(
    ...
    try:
        result = _run_async(
            _cdp_call_serialized_for_cloak(
                endpoint, method, call_params, target_id, safe_timeout, effective_task_id
            )
        )
    except asyncio.TimeoutError as exc:
        return tool_error(
            f"CDP call timed out after {safe_timeout}s: {_redact_cdp_output(str(exc))}",
            method=method,
        )
        ...
        return tool_error(
            f"WebSocket error talking to CDP at {redact_cdp_url(endpoint)}: {_redact_cdp_output(str(exc))}. The "
            "browser may have disconnected — try '/browser connect' again.",
            method=method,
        )
    except Exception as exc:  # pragma: no cover — unexpected
        logger.error("browser_cdp unexpected error: %s", _redact_cdp_output(str(exc)))
        return tool_error(
            f"Unexpected error: {type(exc).__name__}: {_redact_cdp_output(str(exc))}",
            method=method,
        )
