...
def _redact_browser_diagnostics(value: Any) -> Any:
    """Redact secrets from agent-browser output at its public boundary."""
    if isinstance(value, str):
        return _sanitize_url_for_logs(value)
    if isinstance(value, list):
        return [_redact_browser_diagnostics(item) for item in value]
    if isinstance(value, tuple):
        return tuple(_redact_browser_diagnostics(item) for item in value)
    if isinstance(value, dict):
        return {key: _redact_browser_diagnostics(item) for key, item in value.items()}
...
def _format_browser_timeout_error(
    ...
    detail = _sanitize_url_for_logs(stderr or stdout or "").strip()
    if detail:
...
def _get_cdp_override(task_id: Optional[str] = None) -> str:
    ...
    Precedence is:
    1. Task-scoped Cloak lease (when ``task_id`` is known)
    2. ``BROWSER_CDP_URL`` env var — but if ``CLOAK_ACTIVE_TASK_ID`` is set,
       only the owning task may claim it (prevents task-B opening task-A's
       profile / proxy)
    ...
    endpoint.
    """
    # 1) Prefer a lease bound to this task (parallel-safe).
    if task_id is not None:
        try:
            ...
            for key in (
                ...
                lease = session_leases.get(key)
                if lease and lease.cdp_url:
                    return _resolve_cdp_override(lease.cdp_url)
        except Exception as e:
            logger.debug("Cloak lease lookup failed for %s: %s", task_id, e)

    env_override = os.environ.get("BROWSER_CDP_URL", "").strip()
    if env_override:
        active_task = os.environ.get("CLOAK_ACTIVE_TASK_ID", "").strip()
        if active_task and task_id is not None:
            bare = _bare_task_id_for_session_key(task_id)
            if active_task not in {str(task_id), bare}:
                logger.debug(
                    "Ignoring BROWSER_CDP_URL owned by task %s while resolving task %s",
                    active_task,
                    task_id,
                )
                env_override = ""
...
def _ensure_cdp_supervisor(task_id: str) -> None:
    ...
    """
    # Always resolve with task_id so Cloak leases / CLOAK_ACTIVE_TASK_ID
    # isolation apply — never watch a sibling task's profile/proxy.
...
def _stop_cdp_supervisor(task_id: str) -> None:
    ...
    try:
        ...
        logger.debug("CDP supervisor stop for task=%s failed (non-fatal): %s", task_id, _sanitize_url_for_logs(exc))
...
def _explicit_cloud_provider_key() -> Optional[str]:
    """Return the configured ``browser.cloud_provider`` key, or None if unset.

    Used by fail-closed session creation: an explicitly selected provider
    (e.g. ``cloak``) must not silently fall back to local Chromium.
    """
    try:
        from hermes_cli.config import read_raw_config
        ...
        if isinstance(browser_cfg, dict) and "cloud_provider" in browser_cfg:
            return normalize_browser_cloud_provider(browser_cfg.get("cloud_provider"))
    except Exception:
        return None
    return None
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    try:
                ...
                if _is_legacy_provider_registry_overridden():
                    ...
                    if resolved is None:
                        ...
                        logger.warning(
                            ...
                            "config key spelling).",
                            _redact_browser_diagnostics(provider_key),
                        )
            except Exception as exc:
                logger.warning(
                    "Failed to instantiate explicit cloud_provider %r: %s; will retry on next call",
                    _redact_browser_diagnostics(provider_key),
                    _redact_browser_diagnostics(str(exc)),
                )
                return None
    except Exception as e:
        ...
        logger.debug("Could not read cloud_provider from config: %s", _redact_browser_diagnostics(str(e)))
    ...
    if resolved is None:
        ...
        try:
            ...
            if fallback_provider.is_configured():
                ...
                if fallback_provider.is_configured():
                    resolved = fallback_provider
        except Exception as exc:  # pragma: no cover - defensive: never poison cache
            logger.debug("Cloud provider auto-detect failed: %s", _redact_browser_diagnostics(str(exc)))
            return None
        ...
        if provider is None:
            ...
            try:
                ...
                if not session_info or not isinstance(session_info, dict):
                    raise ValueError(f"Cloud provider returned invalid session: {_redact_browser_diagnostics(session_info)!r}")
                ...
                # Cloak Edition / CDP plan: when the operator explicitly selected
                # a cloud provider (e.g. cloak), do NOT silently open local
                # Chromium — that bypasses stealth profiles and confuses diagnosis.
                explicit_provider = _explicit_cloud_provider_key()
                if explicit_provider and explicit_provider not in {"local", "auto", ""}:
                    logger.error(
                        "Explicit cloud provider %s (%s) failed for task %s: %s",
                        explicit_provider, provider_name, task_id, _redact_browser_diagnostics(e),
                    )
                    raise RuntimeError(
                        f"Explicit browser.cloud_provider={explicit_provider!r} "
                        f"({provider_name}) failed: {_redact_browser_diagnostics(e)}. "
                        f"Fail-closed: local Chromium fallback is disabled for "
                        f"explicit providers."
                    ) from e
                logger.warning(
                    "Cloud provider %s failed (%s); attempting fallback to local "
                    "Chromium for task %s",
                    provider_name, _redact_browser_diagnostics(e), task_id,
                )
                try:
                    session_info = _create_local_session(task_id)
                except Exception as local_error:
                    raise RuntimeError(
                        f"Cloud provider {provider_name} failed ({_redact_browser_diagnostics(e)}) and local "
                        f"fallback also failed ({_redact_browser_diagnostics(local_error)})"
                    ) from e
                # Mark session as degraded for observability
                if isinstance(session_info, dict):
                    session_info = dict(session_info)
                    session_info["fallback_from_cloud"] = True
                    session_info["fallback_reason"] = _redact_browser_diagnostics(e)
...
def _run_browser_command(
    ...
    try:
        ...
        logger.warning("Failed to create browser session for task=%s: %s", task_id, _redact_browser_diagnostics(e))
        return {"success": False, "error": f"Failed to create browser session: {_redact_browser_diagnostics(e)}"}
        ...
        try:
            ...
            if stderr and stderr.strip():
                logger.warning(
                    ...
                    command,
                    _redact_browser_diagnostics(stderr.strip()[:500]),
                )
            ...
            if stderr and stderr.strip():
                ...
                logger.log(level, "browser '%s' stderr: %s", command, _redact_browser_diagnostics(stderr.strip()[:500]))
            ...
            if not stdout_text and returncode == 0 and command not in _EMPTY_OK_COMMANDS:
                ...
                try:
                    ...
                    raw = _redact_browser_diagnostics(stdout_text[:2000])
                ...
                safe_error_msg = _redact_browser_diagnostics(error_msg)
                logger.warning("browser '%s' failed (rc=%s): %s", command, returncode, safe_error_msg[:300])
                result = {"success": False, "error": safe_error_msg}
            else:
                result = {"success": True, "data": {}}

    except Exception as e:
        logger.warning("browser '%s' exception: %s", command, _redact_browser_diagnostics(e))
        result = {"success": False, "error": _redact_browser_diagnostics(e)}
    ...
    fallback_reason = _redact_browser_diagnostics(_lightpanda_fallback_reason(engine, command, result))
    if fallback_reason:
        logger.info(
        ...
        return _redact_browser_diagnostics(_annotate_lightpanda_fallback(fallback_result, fallback_reason))

    return _redact_browser_diagnostics(result)
...
def _redact_browser_output(value: Any) -> Any:
    ...
    if isinstance(value, str):
        # JS output can contain authenticated CDP endpoints. Apply content
        # redaction first, then the strict URL policy used at this boundary.
...
def _browser_eval(expression: str, task_id: Optional[str] = None) -> str:
    ...
    try:
        ...
        logger.debug(
            ...
            _redact_browser_diagnostics(exc),
        )
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    try:
        ...
        safe_error = _redact_browser_diagnostics(str(e))
        logger.warning("browser_vision failed: %s", safe_error)
        error_info = {"success": False, "error": f"Error during vision analysis: {safe_error}"}
