...
# ---------------------------------------------------------------------------
#
# Two trust surfaces, two scanners:
#
#   1. User-supplied cron prompt (small, written as a directive). It is scanned
#      for secret access/exfiltration, SSH persistence, destructive commands,
#      and invisible Unicode at create/update time and at runtime.
#
...
#
#      The runtime scanner preserves the same secret/exfiltration and host
#      safety protections, while allowing jailbreak and behavioral
#      prompt-injection text in both user prompts and skill content.
#
# Both scanners share the invisible-unicode check and the GitHub Authorization
# header exemption.

# Destructive patterns retained alongside the shared secret/exfiltration scan.
_CRON_THREAT_PATTERNS = [
    (r'rm\s+-rf\s+/', "destructive_root_rm"),
    (r'\bmkfs\b', "format_filesystem"),
    (r'\bdd\s+.*if=.*of=/dev/', "disk_overwrite"),
]
...
from tools.threat_patterns import (
    INVISIBLE_CHARS as _CRON_INVISIBLE_CHARS,
    scan_for_exfil as _scan_for_exfil,
)
...
def _scan_cron_prompt(prompt: str) -> str:
    """Scan the USER-SUPPLIED cron prompt for retained safety threats.

    Jailbreak, prompt-injection, role-play, and red-team text are allowed.
    Secret access/exfiltration, SSH persistence, destructive commands, and
    invisible Unicode remain blocked. Used at job create/update time and as a
    runtime defense-in-depth for prompts authored before the scanner existed.
    """
    ...
    findings = _scan_for_exfil(_strip_legitimate_emoji_zwj(prompt_to_scan))
    if findings:
        return (
            f"Blocked: prompt matches protected pattern '{findings[0]}'. "
            "Cron prompts may contain jailbreak text but not secret access, "
            "exfiltration, SSH persistence, or destructive payloads."
        )
    for pattern, pid in _CRON_THREAT_PATTERNS:
        if re.search(pattern, prompt_to_scan, re.IGNORECASE):
            return f"Blocked: prompt matches protected pattern '{pid}'."
...
def _scan_cron_skill_assembled(assembled: str) -> tuple[str, str]:
    ...
    Jailbreak and behavioral prompt-injection text is allowed. Skill content
    still receives the shared secret/exfiltration and destructive-command
    scan. Invisible Unicode is sanitized because this is already-installed
    content, so a stray copy-paste character does not permanently kill a job.
    ...
    for pattern, pid in _CRON_THREAT_PATTERNS:
        if re.search(pattern, prompt_to_scan, re.IGNORECASE):
            return cleaned, f"Blocked: prompt matches protected pattern '{pid}'."
    for pattern, pid in _CRON_EXFIL_COMMAND_PATTERNS:
        if re.search(pattern, prompt_to_scan, re.IGNORECASE):
            return cleaned, f"Blocked: prompt matches protected pattern '{pid}'."
    return cleaned, ""
