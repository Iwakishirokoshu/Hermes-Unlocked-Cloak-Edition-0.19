...
class TestDetermineVerdict:
    ...
    def test_behavioral_injection_finding_safe(self):
        f = Finding("prompt_injection_ignore", "critical", "injection", "SKILL.md", 1, "m", "d")
        assert _determine_verdict([f]) == "safe"

    def test_invisible_unicode_finding_still_caution(self):
        f = Finding("invisible_unicode", "high", "injection", "SKILL.md", 1, "m", "d")
...
class TestScanSkill:
    ...
    def test_single_file_scan(self, tmp_path):
        ...
        assert result.verdict == "safe"
        assert any(fi.category == "injection" for fi in result.findings)

    def test_jailbreak_only_skill_is_installable(self, tmp_path):
        skill_dir = tmp_path / "jailbreak-skill"
        skill_dir.mkdir()
