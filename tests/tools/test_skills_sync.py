...
class TestSyncLock:
    def test_serializes_concurrent_syncs_in_one_profile(self, tmp_path):
        """A second startup waits instead of racing manifest and skill moves."""
        ...
        def run_sync():
            results.append(sync_skills(quiet=True))

        with patch("tools.skills_sync.HERMES_HOME", tmp_path), \
                patch("tools.skills_sync._sync_skills_unlocked", side_effect=fake_sync):
...
class TestComputeRelativeDest:
    def test_preserves_category_structure(self):
        ...
        assert dest.name == "axolotl"
        assert dest.parent.name == "mlops"
...
class TestResetBundledSkill:
    ...
    def test_current_copy_self_heals_stale_manifest(self, tmp_path):
        """A current bundled copy re-baselines a stale manifest without a reset."""
        bundled = self._setup_bundled(tmp_path)
        skills_dir = tmp_path / "user_skills"
        manifest_file = skills_dir / ".bundled_manifest"
        ...
        assert "google-workspace" not in result["user_modified"]
        assert "google-workspace" not in result["updated"]
